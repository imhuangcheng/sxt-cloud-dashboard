...
- 同时计算日K和15分钟K的 SXT。
- SXT 计算使用 `JYD1`、`JYD2`、`JYD3` 和 `SXT` 完整公式链。
- 仅当 `日K SXT = 2` 且 `15分钟K SXT = 2` 时触发 Server 酱和企业微信通知；企业微信同时推送 15 分钟 K 线截图。
...
没有配置 `SERVERCHAN_SEND_KEY` 或 `WECHAT_WORK_BOT_URL` 时，程序不会崩溃；满足通知条件时会打印 warning 并将通知记录为失败，便于补齐配置后重试。
...
Token 需要允许读取/写入 Contents，并允许网页保存股票池配置。页面“刷新”按钮只重新读取 GitHub Actions 已提交的结果，不会触发扫描或发送通知；通知只在 GitHub Actions 实际运行 `scripts/run.py` 并产生新的 ALERT 信号时发送。
...
每次监控运行后会提交 `data/latest_signals.json`、`data/latest.json`、`data/alert_history.json` 和 `data/status.json`。即使没有触发信号或 K 线时间没有前进，也会更新 `status.json` 的 `last_scan_time`、`next_scan_time` 和 `message`。如果监控步骤失败，Actions 会写入一份 `workflow=failed` 的 `data/status.json`，供首页状态卡片展示。通知频率以该 workflow 的实际运行结果为准，不由前端刷新频率决定。
...
`SERVERCHAN_SEND_KEY` 填写你的 Server 酱 sendkey；`WECHAT_WORK_BOT_URL` 填写企业微信群机器人的 webhook 地址。两者都应配置在 GitHub Actions Secrets 中，不要写入仓库。
...
因此 GitHub Actions 每次提交新的数据文件后，Netlify 可以展示最新结果；前端读取 JSON 时会追加时间戳参数，避免读取旧缓存。页面自动刷新只读取这些已提交文件，不会调用扫描触发接口。

## 重要文件

- `scripts/fetch_data.py`：按 `simonlin1212/a-stock-data` 的行情层取数，使用腾讯日K/15分钟K和腾讯实时行情名称。
- `scripts/calc_sxt.py`：计算 SXT 指标。
- `scripts/notifier.py`：Server 酱、企业微信通知和去重记录。
- `scripts/chart.py`：生成企业微信使用的 15 分钟 K 线 PNG 截图。
...
云端监控使用以下 TongDaXin 公式末端逻辑，日K和15分钟K共用同一计算实现：

```text
JYD1:=IF(BLDI1 OR BLDI2 OR BLDI3,1,IF(YJDING2,-1,0));
JYD2:=VALUEWHEN(JYD1,JYD1);
JYD3:=IF(JYD2=1 AND C>YJDIP0,1,-1);
SXT:(JYD3+DB1)*IF(C>YJDINGP0 AND DB1=-1,-1,1);
```
